...
def card_to_number(card):
    suit, _, numerical_value = adt.get_card_info(card)
    return numerical_value + (13 if suit == "spades" else 0)
...
def move_card(deck, card_index, move_distance):
    ...
    if card_index + move_distance < 0 or card_index + move_distance > adt.count_cards(
        deck
    ):
        raise Exception(
            f"Trying to move {card_index} up/down {move_distance} (len is {adt.count_cards(deck)})"
        )
    ...
    if move_distance > 0:
        deck_before_target_index, deck_after_target_index = adt.cut_deck(
            deck_after_target, move_distance - 1
        )
        ...
        deck_before_target_index, deck_after_target_index = adt.cut_deck(
            deck_before_target, adt.count_cards(deck_before_target) + move_distance - 1
        )
...
def generate_key_letter(deck):
    ...
    bottom_deck, bottom_card = adt.cut_deck(
        bottom_deck, adt.count_cards(bottom_deck) - 2
    )
